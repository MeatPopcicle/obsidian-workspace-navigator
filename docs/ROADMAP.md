...
### Future
- [ ] **Create workspace/group from a folder** — right-click a directory in the file explorer and create a group, a workspace, or both, named for that folder. NOT a scope or lock (that's Scoped Spaces): the folder only seeds the name and optionally a first note; files are opened manually afterward. Feasibility study done 2026-08-18 (vault: `300 Technological/70 Projects/Workspace Navigator Folder Context Menu Feasibility.md`), verdict feasible S/M; the primitives it needs (`moveWorkspaceToGroup`, tabs-creation in empty layouts, the `file-menu` hook already firing for folders) all exist as of v2.22. Proposed UX: folder context submenu with create-workspace / create-group / create-both, no auto-switch. Remaining design bits: name-collision counter (reuse existing pattern), optional folder-note seeding.
